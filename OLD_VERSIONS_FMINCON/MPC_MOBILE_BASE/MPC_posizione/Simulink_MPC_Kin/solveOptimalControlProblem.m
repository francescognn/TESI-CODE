...
[u V] = fmincon(@(u) costfunction(runningcosts, terminalcosts, system, ... 
     ...
     constraintseval(constraints,terminalconstraints, system, N, T, t0, x0, xd, u,j,u0));
...
function [c,ceq] = constraintseval ...
        (constraints, terminalconstraints, system, N, T, t0, x0, xd, u,j,u0)
   ...
   [x] = computeOpenloopSolution(system, N, T, t0, x0, u);
    ...
    for k=1:N
        ...
        % era valutato su u(:,k)
        du=abs(ulong(:,k+1)-ulong(:,k));
        if k==N-j+1
            j=1;
            [cnew, ceqnew] = constraints(du,x(k,:),u(:,k),N,j,xd(k,:));
        else
            j=0;
            [cnew, ceqnew] = constraints(du,x(k,:),u(:,k),N,j,xd(k,:));
        end

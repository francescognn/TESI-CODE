...
u0=zeros(8,N);
tmeasure=0;
xmeasure= [ 0 0 0 0 pi/2 pi/2 pi/4 pi/4 pi/4]; 
...
cost = [x(:,1:9)-xs]*Q*[x(:,1:9)-xs]'+u.'*R*u;%+(xs-xd)*T*(xs-xd)';

end

function cost = terminalcosts(t, x, xs)
    P=eye(9)*0;
    cost = [x(:,1:9)-xs]*P*[x(:,1:9)-xs]';
...
function [y] = system(t, x, u, T)
    ...
    G = [cos(th), 0 ; 
         sin(th), 0 ; 
         0,       1 ];
    P = [G , zeros(3,6); zeros(6,2), eye(6)];
    x=x.';
    y(1:9,1) = x(1:9,1) + T.*P*u(1:8,1);
    y=y.';

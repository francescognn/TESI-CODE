...
classdef UR5Display < handle
    ...
    methods (Access = private)
        function key_press_cb(obj, ~, event_data)
            ...
            obj.draw_configuration(obj.q_display,  T01);
...
%             [obj.v_wrist3, obj.f_wrist3, obj.n_wrist3, ~] = stlReadBinary(fullfile(mesh_base_dir, 'gripper2.stl'));
            [obj.v_tool, obj.f_tool, obj.n_tool, ~] = stlReadBinary(fullfile(mesh_base_dir, 'gripper2.stl'));
            ...
            obj.precomp_matrices{7} = makehgtform('translate', [0.085 obj.offset_tool 0]);

            obj.control_mode = UR5Display.JOINT_MODE;
            obj.q_display = zeros(1, 9);
            obj.q_delta = 0.05;
%             obj.draw_configuration(obj.q_display, T);
        ...
        function draw_configuration(obj, q, T01)
            if true == any(abs(q(3:end)) > pi)
                warning('UR5 Display : q_new is overlimits');
            end
            
            obj.q_display = q;
            T_base = ROTZ(q(3));
            T_base = [[T_base; [0,0,0]], [q(1); q(2); 0; 1] ];
%             T01 = in_mm.T_ur5_mobilebase;
            obj.g_base.Matrix = T_base*T01;
            
            
            
            result = obj.ur5_kin.collision_balls(q(4:end));

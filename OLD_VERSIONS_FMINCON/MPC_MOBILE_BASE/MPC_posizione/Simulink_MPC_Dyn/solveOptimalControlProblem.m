...
% 
% Tim = timer('startdelay',T*0.95,'timerfcn',@(src,evt)setappdata(0,'FMINCONsStopFlag',true)); %after 10 seconds change the flag to true
% 
% start(Tim); %start the timer, T minus 10 seconds!
%     
...
%         if k<N
        du=abs(ulong(:,k+1)-u(:,k));
        [cnew, ceqnew] = constraints(du,x(k,:),u(:,k),N,j);
%         else 
%         [cnew, ceqnew] = constraints([0.1;0.1],x(k,:),u(:,k),N,j);
%         end
